...
// Elements selectors
let navItems: Element[] = Array.from(document.querySelectorAll("nav .nav-item a"));
let landingImgs: Element[] = Array.from(document.querySelectorAll(".landing .img-group img"))

// loader
let loader: any = document.querySelector(".loader-container");
  loader.style.display = "none";

// navbar
function classTogglerGroup(array: Element[], className: string, event: string | undefined = undefined):void{
...
// landing
function indexSwicher(order: number): void{
  let [img1, img2, img3]: any[]= landingImgs
      if(order == 0){
...
hiddenGroups.forEach((group)=>{
  let element: any = group as Element;
  // getting the custom items selector if found
  let itemSelector: string = "";
  element.getAttribute("hidden-item-selector")? element.getAttribute("hidden-item-selector"): Array.from(element.children).forEach((item:any) => { item.classList.contains("hidden-item")?itemSelector = ".hidden-item": ""});
  // getting all the items of the group by the itemSelector variables
  let groupItems: Element[] = [];
  ...
  groupItems.forEach((item:any,index)=>{
...
*/
let activeTitles: object[] = [];
const titleObserver = new IntersectionObserver((entries=>{
  entries.forEach((entry)=>{
    if (entry.isIntersecting){activeTitles.map((title: any)=> title.element == entry.target? title.state = true: "")
  }else{
    activeTitles.map((title: any)=> title.element == entry.target? title.state = false: "")
  }
  })
}))
const subTitleLines: Element[] = Array.from(document.querySelectorAll('.main-title .sub-title-container'));
...
document.addEventListener('scroll',()=>{
  activeTitles.forEach((title: any)=>{
    if(title.state){
      let percentageFromTop: number =title.element.getBoundingClientRect().top/window.innerHeight;
      let percentage: number = Math.abs(percentageFromTop - 1);
      percentage = Math.round(percentage*100);
      title.element.querySelectorAll('.line-left .filler, .line-right .filler').forEach((line:any)=> line.setAttribute('style',`width: ${percentage}%; min-width: 20px`))
    }
  })
});
...
/*
  Portfolio
*/
let allPortElements: Element[] = [];
let allPortData: object[] = [
...
function PortItemsInit(){
  for(let i = 0; i< allPortData.length; i++){
    let itemData: any = allPortData[i]
    let filters: [] = itemData.filterClasses;
    let title: string = itemData.title;
    let description: string = itemData.description;
    let imgPath: string = itemData.imgPath;
    let infoPath: string = itemData.infoPath;

    // badges
    let badges: string = '';
    if(itemData.badges){for(let i = 0; i<itemData.badges.length;i++){badges = badges+`<span class="badge bg-${itemData.badges[i].bg}">${itemData.badges[i].text}</span>`}}
...
PortItemsInit()
function appendPortItems(num: number){
...
// countDown
let countdownDate: any = new Date("Dec 22, 2023 15:37:25");
let days: any = document.querySelector(".events .days span")
let hours: any = document.querySelector(".events .hours span")
let minutes: any = document.querySelector(".events .minutes span")
let seconds: any = document.querySelector(".events .seconds span")
...
/*
  contact section
*/
document.getElementById('contactSend').addEventListener('click', (e: any)=> {
